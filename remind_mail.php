...
$semester = ($current_month >= $year_start_month) ? "Even" : "Odd";
    ...
    if (isset($_POST['message']) && isset($_POST['type'])) {
        ...
        if ($type === 'backlogs') {
            if ($message === 'auto') {
                ...
                sendEmails($type, $subject, $message);
            } else {
...
function sendEmails($type, $subject, $message) {
    ...
    try {
        ...
        if($type === 'backlogs') {
            ...
            // Fetch students with minor courses from the database
            $result = $mysqli->query("SELECT roll, minor_courses FROM students WHERE minor_courses IS NOT NULL");
            if ($result->num_rows > 0) {
                // Loop through each student
                while ($row = $result->fetch_assoc()) {
                    $roll = $row['roll'];
                    $minor_courses = explode(',', $row['minor_courses']);
                    $course_list = [];
                    foreach ($minor_courses as $course) {
                        $course = trim($course); // Trim whitespace

                        // Fetch semester information for the minor course
                        $course_info = $mysqli->query("SELECT course_name,year,credit,credit_structure FROM courses WHERE course_code = '$course' AND semester = '$semester'");
                        if ($course_info->num_rows > 0) {
                            $course_row = $course_info->fetch_assoc();
                            $course_name = $course_row['course_name'];
